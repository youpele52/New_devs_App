...
def make_user(tenant_id: str) -> AuthenticatedUser:
...
@pytest_asyncio.fixture
async def client(db_session_factory, auth_context, monkeypatch):
    fake_redis = FakeRedis()
    monkeypatch.setattr(revenue_cache, "redis_client", fake_redis)

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://testserver") as test_client:
        yield test_client, auth_context, fake_redis, db_session_factory
